...
public:
	...
	* 
	...
	// EMontageGraphDrawPinTypes DrawPinTypes = EMontageGraphDrawPinTypes::Both;
	 ...
	 *
	 * This is set by default to an internal DataTables (that you can find in `/MontageGraph/Xelu_Icons/`) that setup texture icons for every keyboard and gamepad key.
	 *
	 ...
	 */
	// UPROPERTY(config, EditDefaultsOnly, Category = "Icons", meta = (RequiredAssetDataTags = "RowStructure=MontageGraphInputsMetaData"))
	...
	// EMontageGraphIconType IconPreference = EMontageGraphIconType::Keyboard;

	/** Size of Icons when drawing edges (transitions) in MontageGraphs */
	UPROPERTY(config,
		EditAnywhere,
		Category = "Icons",
	...
	// TMap<TSoftClassPtr<UAnimNotifyState>, FMontageGraphNotifyStateAutoSetup> NotifyStates;
	//
