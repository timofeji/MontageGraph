...
#define LOCTEXT_NAMESPACE "MontageGraphDragAction"
...
void FMontageGraphDragDropAction::HoverTargetChanged()
{
	...
	else if (const UMGEdNode* TargetNode = Cast<UMGEdNode>(GetHoveredNode()))
	{
...
void FMontageGraphDragDropAction::AppendConnectionResponsesForHoveredNode(const UMGEdNode* TargetNode, TArray<FPinConnectionResponse>& OutResponses)
{
	...
	for (const UEdGraphPin* StartingPin : StartingPins)
	{
		...
		{
			Response = FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW, LOCTEXT("PinError", "Not a valid Montage Graph Node"));
		}
