#pragma once
#include "ActionGraphEditorSettings.generated.h"
...
UENUM()
enum class EHBActionGraphDrawPinTypes : uint8
{
	/** Setup Event */
...
UCLASS(Config = Game,
	defaultconfig)
class ACTIONGRAPHEDITOR_API UActionGraphEditorSettings : public UObject
{
	GENERATED_BODY()

public:
	UActionGraphEditorSettings();
	...
	// EHBActionGraphDrawPinTypes DrawPinTypes = EHBActionGraphDrawPinTypes::Both;
	 ...
	 *
	 * This is set by default to an internal DataTables (that you can find in `/HBActionGraph/Xelu_Icons/`) that setup texture icons for every keyboard and gamepad key.
	 *
	 ...
	 */
	// UPROPERTY(config, EditDefaultsOnly, Category = "Icons", meta = (RequiredAssetDataTags = "RowStructure=HBActionGraphInputsMetaData"))
	...
	// EHBActionGraphIconType IconPreference = EHBActionGraphIconType::Keyboard;

	/** Size of Icons when drawing edges (transitions) in HBActionGraphs */
	UPROPERTY(config,
		EditAnywhere,
		Category = "Icons",
	...
	// TMap<TSoftClassPtr<UAnimNotifyState>, FHBActionGraphNotifyStateAutoSetup> NotifyStates;
	//
