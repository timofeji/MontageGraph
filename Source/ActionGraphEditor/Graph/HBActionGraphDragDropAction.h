...
class ACTIONGRAPHEDITOR_API FHBActionGraphDragDropAction : public FGraphEditorDragDropAction
{
public:
	DRAG_DROP_OPERATOR_TYPE(FACEGraphDragAction, FGraphEditorDragDropAction)

	FHBActionGraphDragDropAction(const TSharedRef<SGraphPanel>& GraphPanel, TArray<FGraphPinHandle>& DraggedPins);
	static TSharedRef<FHBActionGraphDragDropAction> New(const TSharedRef<SGraphPanel>& GraphPanel, TArray<FGraphPinHandle>& InStartingPins);
...
private:
	...
	void AppendConnectionResponsesForHoveredNode(const UActionGraphEdNode* TargetNode, TArray<FPinConnectionResponse>& OutResponses);
