...
UActionGraphEdNode::UActionGraphEdNode()
{
}

UEdGraphPin* UActionGraphEdNode::GetInputPin() const
{
	if (Pins.Num() == 0)
	{
		return nullptr;
...
UEdGraphPin* UActionGraphEdNode::GetOutputPin() const
{
...
FLinearColor UActionGraphEdNode::GetBackgroundColor() const
{
	if (!RuntimeNode)
	{
	...
	// const UHBActionGraphProjectSettings* Settings = FHBActionGraphUtils::GetPluginProjectSettings();
...
void UActionGraphEdNode::AllocateDefaultPins()
{
	CreatePin(EGPD_Input, UActionGraphPinNames::PinCategory_MultipleNodes, FName(), UActionGraphPinNames::PinName_In);
	CreatePin(EGPD_Output, UActionGraphPinNames::PinCategory_MultipleNodes, FName(), UActionGraphPinNames::PinName_Out);
}

void UActionGraphEdNode::PrepareForCopying()
{
	RuntimeNode->Rename(nullptr, this, REN_DontCreateRedirectors | REN_DoNotDirty);
}

FText UActionGraphEdNode::GetNodeTitle(const ENodeTitleType::Type TitleType) const
{
	if (!RuntimeNode)
	{
...
void UActionGraphEdNode::AutowireNewNode(UEdGraphPin* FromPin)
{
...
void UActionGraphEdNode::ValidateNodeDuringCompilation(FCompilerResultsLog& MessageLog) const
{
	...
	// const bool bIsSequence = RuntimeNode->IsA(UHBActionGraphNodeSequence::StaticClass());
	//
	// if (bIsSequence)
	// {
...
void UActionGraphEdNode::PostEditUndo()
{
	Super::PostEditUndo();
}

void UActionGraphEdNode::UpdateTime(const float DeltaTime)
{
	if (!RuntimeNode)
	{
	...
	const UActionGraphNode* DebuggedNode = GetDebuggedNode();
	if (!DebuggedNode)
	{
		bIsDebugActive = bWasDebugActive = false;
...
void UActionGraphEdNode::UpdateErrorReporting(USkeletalMesh* InSkeletalMesh, const FText ErrorMessage) const
{
...
float UActionGraphEdNode::WasActiveTime() const
{
	return DebugTotalTime - DebugElapsedTime;
}

void UActionGraphEdNode::UpdateTimeRuntimeNodeChanged(const float DeltaTime, const float DebugMaxTime)
{
	// Debugged node changed, mark as was active if we were active
...
void UActionGraphEdNode::UpdateTimeCurrentNode(const float DeltaTime, const float DebugMaxTime)
{
...
const UActionGraphNode* UActionGraphEdNode::GetDebuggedNode() const
{
	UHBActionGraphEdGraph* HBActionGraph = Cast<UHBActionGraphEdGraph>(GetOuter());

	if (HBActionGraph && HBActionGraph->Debugger.IsValid())
	{
		FActionGraphDebugger* Debugger = HBActionGraph->Debugger.Get();
